...
MPIARGS="--allow-run-as-root --mca osc ^ucx -np ${NUM_PROCESS} --map-by ppr:${PPN}:node -hostfile /root/hostfile${NUM_NODES} -x CXL_SHIM_ALLOC=${CXL_SHIM_ALLOC} -x CXL_SHIM_COPY_SEND=${CXL_SHIM_COPY_SEND} -x CXL_SHIM_COPY_RECV=${CXL_SHIM_COPY_RECV} -x CXL_DAX_PATH=${CXL_DAX_PATH} -x CXL_DAX_RESET=${CXL_DAX_RESET} -x GLIBC_TUNABLES=${GLIBC_TUNABLES} -x LD_PRELOAD=${MPI_SHIM_LIB}"
