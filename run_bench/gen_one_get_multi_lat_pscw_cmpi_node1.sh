...
# Generate a single run_#{use_case}.sh containing all one-sided PSCW latency benchmark commands
# (all NUM_PROCESS combinations) by calling gen_eval.sh with a spec file.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

BENCH_NAME="osu_get_multi_lat_pscw"
USE_CASE_BASE="osu_get_multi_lat_pscw_cmpi_node1_20260302"
NET_CONFIG="cmpi"
NUM_NODES="1"
