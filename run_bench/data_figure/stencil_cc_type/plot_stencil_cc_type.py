...
def collect_data():
    """Collect timing data from the latest timestamp only: {size: {cc_type: {nprocs: time}}}."""
    data = {}
    for size in SIZES:
        data[size] = {}
        for cc in CC_TYPES:
            ...
            for nprocs in PROCESS_COUNTS:
                ...
                # Use only the latest timestamp log
                for log in reversed(logs):
                    t = parse_time_from_log(log)
                    if t is not None:
                        data[size][cc][nprocs] = t
                        break
    return data
...
def plot_grouped_bars(data):
    ...
    for idx, size in enumerate(SIZES):
        ...
        ax.set_xticklabels([str(p) for p in PROCESS_COUNTS], fontsize=10)
        ax.set_xlabel("Number of Processes", fontsize=11)
        ax.set_ylabel("Normalized Execution Time\n(relative to No CC)", fontsize=10)
        ax.set_title(f"Grid Size {size}x{size}", fontsize=12, fontweight="bold")
