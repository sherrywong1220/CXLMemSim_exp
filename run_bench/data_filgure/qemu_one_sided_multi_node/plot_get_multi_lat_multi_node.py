#!/usr/bin/env python3
"""
Plot osu_get_multi_lat from results_qemu (mpi_cxl, nocc).
...
RESULTS_BASE = os.path.join(os.path.dirname(__file__), "../../results_qemu/osu_get_multi_lat/mpi_cxl/nocc")
...
def discover_data():
    ...
    pattern = os.path.join(RESULTS_BASE, "*", "osu_get_multi_lat_mpi_cxl_node*", "*", "output.log")
    ...
    for f in all_files:
        ...
        for i, p in enumerate(parts):
            ...
            if m and "osu_get_multi_lat" in p:
                node = int(m.group(1))
        if node and nprocs:
            key = (node, nprocs)
            latest[key] = f
...
def plot_multi_node(data):
    ...
    axes[0].set_ylabel("Latency (us)", fontsize=25)
    ...
    out_png = os.path.join(OUTPUT_DIR, "get_multi_lat_multi_node.png")
    fig.savefig(out_png, dpi=300, bbox_inches="tight")
    print(f"Saved: {out_png}")

    out_pdf = os.path.join(OUTPUT_DIR, "get_multi_lat_multi_node.pdf")
